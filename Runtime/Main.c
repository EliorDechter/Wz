...
// ---- Embedded SPIR-V shaders (compiled from Runtime/shaders/*.glsl via glslc --target-env=vulkan1.0) ----
#include "shaders/spirv_shaders.h"
...
// Startup timing helpers — collect all timestamps, print at the end of AppInit
#define MAX_STARTUP_TIMINGS 32
static Uint64  _st_times[MAX_STARTUP_TIMINGS];
static const char* _st_labels[MAX_STARTUP_TIMINGS];
static int     _st_count = 0;
#define ST(label) do { _st_times[_st_count] = SDL_GetTicksNS(); _st_labels[_st_count] = (label); _st_count++; } while(0)
#define ST_PRINT() do { \
	for (int _i = 1; _i < _st_count; _i++) { \
		double _ms = (double)(_st_times[_i] - _st_times[_i-1]) / 1e6; \
		SDL_Log("startup %-40s %8.2f ms", _st_labels[_i], _ms); \
	} \
	double _total = (double)(_st_times[_st_count-1] - _st_times[0]) / 1e6; \
	SDL_Log("startup %-40s %8.2f ms", "TOTAL", _total); \
} while(0)

void WSDL_ContextCreate(WSDL_Context* context, unsigned window_width, unsigned window_height)
{
	ST("start");
	SDL_Window* window = SDL_CreateWindow("my window", window_width, window_height, SDL_WINDOW_RESIZABLE | SDL_WINDOW_HIDDEN);
	assert(window);
	ST("CreateWindow");

	// Disable implicit Vulkan layers (OBS/Discord/Steam etc.) to prevent hang in vkEnumerateInstanceExtensionProperties
	//SDL_SetEnvironmentVariable(SDL_GetEnvironment(), "VK_LOADER_LAYERS_DISABLE", "*", true);
	...
	ST("ClaimWindow+Swapchain");

	// Load pre-compiled SPIR-V shaders
	SDL_GPUShader* vs = SDL_CreateGPUShader(device, &(SDL_GPUShaderCreateInfo){
		.code = vert_spv, .code_size = sizeof(vert_spv),
		.entrypoint = "main", .format = SDL_GPU_SHADERFORMAT_SPIRV,
		.stage = SDL_GPU_SHADERSTAGE_VERTEX,
		.num_samplers = 0, .num_uniform_buffers = 1,
	});
	ST("CreateGPUShader(vert)");
	SDL_GPUShader* ps = SDL_CreateGPUShader(device, &(SDL_GPUShaderCreateInfo){
		.code = frag_spv, .code_size = sizeof(frag_spv),
		.entrypoint = "main", .format = SDL_GPU_SHADERFORMAT_SPIRV,
		.stage = SDL_GPU_SHADERSTAGE_FRAGMENT,
		.num_samplers = 1, .num_uniform_buffers = 0,
	});
	ST("CreateGPUShader(frag)");
	assert(vs && ps);
	...
	SDL_GPUVertexAttribute vertex_attrs[] = {
		...
		{ .location = 1, .buffer_slot = 0, .format = SDL_GPU_VERTEXELEMENTFORMAT_FLOAT4, .offset = 8  },
...
SDL_AppResult SDL_AppIterate(void* appstate)
{
...

	{
		...
		if (!window_shown) { SDL_ShowWindow(gui_window.window); window_shown = true; }
