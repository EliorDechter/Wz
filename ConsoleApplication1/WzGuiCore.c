...
WzWidget wz_scroll_box(wzrd_v2 size, unsigned int* scroll, WzWidget parent, const void* tag)
{
	...
	wz_widget_set_border(middle_panel, WZ_BORDER_RAISED);
	...
	unsigned int count, unsigned int* failed)
{
	...
	while (widgets_stack_count)
	{
		...
		else
		{
			...
			if (widget->layout_type== WZ_LAYOUT_HORIZONTAL || widget->layout_type== WZ_LAYOUT_VERTICAL)
			{
				if (widget->layout_type== WZ_LAYOUT_HORIZONTAL)
				{
				...
				else if (widget->layout_type== WZ_LAYOUT_VERTICAL)
				{
				...
				if (widgets_visits[parent_index] == WZ_LAYOUT_STAGE_NON_FLEX_CHILDREN)
				{
					...
					for (int i = 0; i < widget->children_count; ++i)
					{
						...
						if (*constraint_max_cross_axis > margins_cross_axis)
						{
							...
							{
								if (widget->layout_type== WZ_LAYOUT_VERTICAL)
								{
									wz_assert(parent_max_cross_axis_constraints <= wz->window.w);
								}
								else if (widget->layout_type== WZ_LAYOUT_HORIZONTAL)
								{
				...
				else if (widgets_visits[parent_index] == WZ_LAYOUT_STAGE_PARENT)
				{
					...
					if (widget->main_axis_size_type == MAIN_AXIS_SIZE_TYPE_MIN)
					{
						if (widget->constraint_max_h == widget->min_h &&
							widget->layout_type== WZ_LAYOUT_HORIZONTAL)
						{
							wz_log(log_messages, &log_messages_count,
						...
						if (widget->constraint_max_h == widget->min_h &&
							widget->layout_type== WZ_LAYOUT_VERTICAL)
						{
							wz_log(log_messages, &log_messages_count,
			...
			else if (widget->layout_type== WZ_LAYOUT_NONE)
			{
	...
	for (int i = 1; i < count; ++i)
	{
		...
		for (int j = 0; j < widget->children_count; ++j)
		{
			...
			{
				if (widget->layout_type== WZ_LAYOUT_HORIZONTAL && parent_size_w > child->actual_w)
				{
					child->actual_x += (parent_size_w - children_size_main_axis) / 2;
				}
				else if (widget->layout_type== WZ_LAYOUT_VERTICAL && parent_size_h > child->actual_h)
				{
			...
			{
				if (widget->layout_type== WZ_LAYOUT_HORIZONTAL && parent_size_h > child->actual_h)
				{
					child->actual_y += (parent_size_h - child->actual_h) / 2;
				}
				else if (widget->layout_type== WZ_LAYOUT_VERTICAL && parent_size_w > child->actual_w)
				{
